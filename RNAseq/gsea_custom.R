...
pathways <- pathways[grepl("HALLMARK",names(pathways)) | grepl("GLUTATHIONE",names(pathways))]

str(head(pathways))

# perform gsea
fgseaRes <- fgsea(pathways, ranks, minSize=15, maxSize=500, nperm=1e6, nproc=4)
fn <- paste0(outDir,"/summary_custom.txt")
...
fn <- paste0(outDir,"/summary_custom.pdf")
