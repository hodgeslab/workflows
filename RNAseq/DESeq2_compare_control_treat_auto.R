...
maplot <- function (res, thresh=0.10, foldthresh=log2(1.5), labelthresh=1.5, labelsig=F, minCount=0, ...) {
  with(subset(res, (!(padj<thresh & abs(log2FoldChange) > foldthresh) | is.na(padj)) & baseMean > minCount), heatscatter(log10(baseMean), log2FoldChange, pch=16, cex=0.5, colpal="greys", rev=F, main="", ...))
  with(subset(res, padj<thresh & log2FoldChange < -foldthresh & baseMean > minCount), heatscatterpoints(log10(baseMean), log2FoldChange, colpal="blues", rev=F, pch=16, cex=0.5))
  with(subset(res, padj<thresh & log2FoldChange > foldthresh & baseMean > minCount), heatscatterpoints(log10(baseMean), log2FoldChange, colpal="oranges", rev=F, pch=16, cex=0.5))
  if (labelsig) {
    require(calibrate)
