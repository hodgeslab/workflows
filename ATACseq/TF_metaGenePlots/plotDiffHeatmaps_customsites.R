...
colormapControl <- brewer.pal("RdYlBu",n=10)
colormapTreat <- brewer.pal("RdYlBu",n=10)

# colormapControl <- magma(colorLen, direction=-1)
# colormapTreat <- magma(colorLen, direction=-1)

colormapControl <- colorRampPalette(c("#feffea","#fbffb2","#fc8e5b","#280058","#000000"))(colorLen)
colormapTreat <- colorRampPalette(c("#feffea","#fbffb2","#fc8e5b","#280058","#000000"))(colorLen)
...
# most recent committed color choice (light blue/brown)
# colormapControl <- colorRampPalette(c("#cddeee","#c5dee6","#ffffff","#f6c54a","#a42018"))(colorLen)
# colormapTreat <- colorRampPalette(c("#cddeee","#c5dee6","#ffffff","#f6c54a","#a42018"))(colorLen)

# blue black yellow
# colormapControl <- colorRampPalette(c("#389cde","#000000","#fffd46"))(colorLen)
# colormapTreat <- colorRampPalette(c("#389cde","#000000","#fffd46"))(colorLen)

# orange white purple (inverse of blue black yellow, 50% muted orange)
# colormapControl <- colorRampPalette(c("#e3b190","#FFFFFF","#0002B9"))(colorLen)
# colormapTreat <- colorRampPalette(c("#e3b190","#FFFFFF","#0002B9"))(colorLen)

# blue white dark red -- brewer.pal(11,'RdBu')[c(8,6,2)]
c1 <- brewer.pal(11,'RdBu')[7]
c2 <- brewer.pal(11,'RdBu')[6]
c3 <- brewer.pal(11,'RdBu')[2]
...
lowLim <- apply(imgTot,1,function(x) { as.numeric(quantile(x,0.15)) } )
highLim <- apply(imgTot,1,function(x) { as.numeric(quantile(x,0.99)) } )
imgTot <- (imgTot-lowLim)/(highLim-lowLim)
...
# hch to make unsorted, comment out the following lines
...
imgControl[imgControl < 0] <- 0
imgTreat[imgTreat < 0] <- 0
    
# write images
outfile <- paste0(bedName,'_control.tif')                # 100 pixels wide, and
...
outfile <- paste0(bedName,'_treat.tif')                # 100 pixels wide, and
