...
getJasparMotifs2022 <- function(species = "Homo sapiens", 
  ...
  opts <- c(opts, list(...))
  out <- TFBSTools::getMatrixSet('/s1/share/JASPAR2022/JASPAR2022.sqlite3',opts) 
  if (!isTRUE(all.equal(TFBSTools::name(out), names(out)))) 
    names(out) <- paste(names(out), TFBSTools::name(out), sep = "_")
  return(out)
}
motifs <- getJasparMotifs2022()
...
df$deltaScore3 <- df[,tsel[3]] - df[,csel[3]]
df$deltaScore4 <- df[,tsel[4]] - df[,csel[4]]
df$rank <- rank(df$deltaScoreMean,ties.method = "first")
df <- df[order(df$rank,decreasing = F), ]

ymax <- max(abs(df$deltaScoreMean))
df <- melt(df,id.vars = c("rank","motif"), measure.vars = c("deltaScore1","deltaScore2","deltaScore3","deltaScore4","deltaScoreMean"))
...
df$alpha[df$variable == "deltaScore3"] <- 0.2
df$alpha[df$variable == "deltaScore4"] <- 0.2
