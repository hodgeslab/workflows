...
for(i in conditions) {
  ...
  sel <- c(round(0.25*ncol(imgControl)),round(0.5*ncol(imgControl)))
  ...
  par(mar = c(0,0,0,0))
  image(rotate(as.matrix(imgControl)), col=colormapControl,axes = FALSE,breaks=c(seq(0,normConst,length.out=10),100))
  dev.off()
  ...
  par(mar = c(0,0,0,0))
  image(rotate(as.matrix(imgTreat)), col=colormapTreat,axes = FALSE,breaks=c(seq(0,normConst,length.out=10),100))
  dev.off()
}
