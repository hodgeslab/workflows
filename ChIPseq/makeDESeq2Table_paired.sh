...
for SAMPLE in `cat $SAMPLELIST`; do
  ...
  FRAGMENTSBED=$INPUTDIR/${SAMPLE}.bed
  MERGEDPEAKSBED=$OUTPUTDIR/mergedPeaks_count-${SAMPLE}.bed
  BACKGROUNDBED=$OUTPUTDIR/mergedPeaks_background-${SAMPLE}.bed
  BGDENSITYFILE=$OUTPUTDIR/mergedPeaks_bgDensity-${SAMPLE}.txt
  COUNTMINUSBGBED=$OUTPUTDIR/mergedPeaks_countMinusBg-${SAMPLE}.bed
...
for SAMPLE in `cat $SAMPLELIST`; do
  COUNTBED=$OUTPUTDIR/mergedPeaks_count-${SAMPLE}.bed
  COUNTMINUSBGBED=$OUTPUTDIR/mergedPeaks_countMinusBg-${SAMPLE}.bed
