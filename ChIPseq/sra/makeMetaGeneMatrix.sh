...
makeMatrix() {
  ...
  bwtool -keep-bed matrix ${MAXDIST}:${MAXDIST} $LOCALBED -tiled-averages=${TILE} $BIGWIG1 $OUT1
  sed -i 's/NA/0/g' $OUT1
  tr "," "\t" < $OUT1 > $OUT1.tmp
  mv $OUT1.tmp $OUT1

  bwtool -keep-bed matrix ${MAXDIST}:${MAXDIST} $LOCALBED -tiled-averages=${TILE} $BIGWIG2 $OUT2
  sed -i 's/NA/0/g' $OUT2
  tr "," "\t" < $OUT2 > $OUT2.tmp
  mv $OUT2.tmp $OUT2
}

for NAME in treat-increased treat-decreased; do
  BEDFILE=${NAME}.bed
