...
# cfile <- '~/data/hodgeslab/easmith/10x/data/Eric_1/velocyto/Eric_1.loom'
# tfile <- '~/data/hodgeslab/easmith/10x/data/Eric_2/velocyto/Eric_2.loom'

cldat <- read.loom.matrices(cfile)
tldat <- read.loom.matrices(tfile)

## testing

cvel <- rowSums(cldat$unspliced)/(rowSums(cldat$spliced))
tvel <- rowSums(tldat$unspliced)/(rowSums(tldat$spliced))
cvel <- cvel[unique(names(cvel))]
tvel <- tvel[unique(names(tvel))]
df <- data.frame(cvel,tvel,label=names(cvel))
df$label[df$label != 'SMARCA4'] <- NA

p1 <- ggplot(df,aes(x=cvel,y=tvel,label=label)) +
  ...
  xlab("control [u/s]") +
  geom_point(shape=".") +
  geom_text(size=6*linescale, nudge_x=2)

pdf('ratiotest.pdf', height=2.5, width=3)
print(p1)
dev.off()
